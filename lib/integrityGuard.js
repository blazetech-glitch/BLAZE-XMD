 ...
 *      strings "devblaze" and "bmbtz". This catches someone doing a
...
// Keep the guard above the current baseline without requiring legacy bmbtz naming counts.
    ...
    function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                const bmbtzMatches = content.match(/blazetz/g);
                if (devblazeMatches) {
                    filesWithDevbmb++;
...
function verifyIntegrity(projectRoot) {
    ...
    if (
        ...
        filesWithBmbtz < MIN_FILES_REFERENCING_BLAZETZ ||
        totalDevbmb < MIN_TOTAL_OCCURRENCES_DEVBLAZE ||
        totalBmbtz < MIN_TOTAL_OCCURRENCES_BLAZETZ
    ) {
        console.error("❌ INTEGRITY CHECK FAILED");
        console.error("References to \"devblaze\"/\"bmbtz\" across the codebase dropped below the expected minimum —");
        ...
        console.error(
            `  bmbtz:  ${filesWithBmbtz} files / ${totalBmbtz} occurrences (need >= ${MIN_FILES_REFERENCING_BLAZETZ} files / ${MIN_TOTAL_OCCURRENCES_BLAZETZ} occurrences)`
        );
        console.error('Revert those changes back to using "devblaze"/"bmbtz" to continue.');
        process.exit(1);
    }
